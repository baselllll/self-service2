...
class UploadDocumnetAcrchive
{
    public function upload($employeeNumber, $outputFileName_local_word_readyOnly,$file_output_name){
        ...
        $sftp = new SFTP("192.168.15.245");

        // Authenticate with the server using a username and password
        if (!$sftp->login("appwls", "appwls123")) {
            echo "Login failed.";
            return;
        }
...
if (isset($path_url[0])){
    ...
    if(isset($tranc_code[0])) {
        ...
        if ($sftp->put($remoteFilePath, $localFilePath, SFTP::SOURCE_LOCAL_FILE)) {
            try {
                return DB::statement("BEGIN XXAJMI_ARCHIVE_FILE_SSHR ('$employeeNumber', '$file_output_name', $compressedSizeInKB); END;");
